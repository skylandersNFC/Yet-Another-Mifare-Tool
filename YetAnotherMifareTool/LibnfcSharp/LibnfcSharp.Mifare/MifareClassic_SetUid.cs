...
namespace LibnfcSharp.Mifare
{
    public partial class MifareClassic
    {
        public bool SetUid(byte[] uidOrBlock0 = null, bool format = false)
        {
            ...
            {
                _logCallback?.Invoke(LogLevel.Error, "Error: No NFC tag available");
                return false;
            }
            ...
            _logCallback?.Invoke(LogLevel.Information, "Formatting NFC tag...");
            switch (szCL)
            {
                case 1:
                    _logCallback?.Invoke(LogLevel.Debug, $"UID: {abtRawUid[0]:X2}{abtRawUid[1]:X2}{abtRawUid[2]:X2}{abtRawUid[3]:X2}");
                    break;

                case 2:
                    _logCallback?.Invoke(LogLevel.Debug, $"UID: {abtRawUid[1]:X2}{abtRawUid[2]:X2}{abtRawUid[3]:X2}{abtRawUid[4]:X2}{abtRawUid[5]:X2}{abtRawUid[6]:X2}{abtRawUid[7]:X2}");
                    break;

                case 3:
                    _logCallback?.Invoke(LogLevel.Debug, $"UID: {abtRawUid[1]:X2}{abtRawUid[2]:X2}{abtRawUid[3]:X2}{abtRawUid[5]:X2}{abtRawUid[6]:X2}{abtRawUid[7]:X2}{abtRawUid[8]:X2}{abtRawUid[9]:X2}{abtRawUid[10]:X2}{abtRawUid[11]:X2}");
                    break;
            }
            ...
            {
                _logCallback?.Invoke(LogLevel.Warning, "Warning: Unlock command 0x40 failed.");
            }
            else
            {
                if (format)
                ...
                {
                    _logCallback?.Invoke(LogLevel.Information, "NFC tag unlocked.");
                }
                else
                {
                    _logCallback?.Invoke(LogLevel.Warning, "Warning: Unlock command 0x43 failed.");
                }
            }
            ...
            if (TransmitBytes(abtWrite, 4) && TransmitBytes(abtData, 18))
            {
                if (format)
                {
                    for (byte i = 3; i < 64; i += 4)
                    {
						...
						_logCallback?.Invoke(LogLevel.Debug, $"Block {i} formatted successfully.");
