...
namespace LibnfcSharp.Mifare
{
    public partial class MifareClassic
    {
        ...
        {
            _logCallback?.Invoke(LogLevel.Information, "Resetting NFC tag...");
            ...
            foreach (var blockToReset in blocksToReset.OrderBy(x => x))
            {
                ...
                if (currentSector != lastAuthenticatedSector)
                {
                        ...
                        MagicCardType == MifareMagicCardType.GEN_2)
                    {
                        ...
                        {
                            _logCallback?.Invoke(LogLevel.Information, $"Sector {currentSector} authenticated successfully.");
                ...
                {
                    _logCallback?.Invoke(LogLevel.Debug, $"Block {blockToReset} reset successfully.");
                }
                else
                {
            ...
            _logCallback?.Invoke(LogLevel.Information, "NFC tag reset successfully.");
